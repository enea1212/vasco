...
class _MapPageState extends State<MapPage> {
  ...
  void didChangeDependencies() {
    ...
    final newProvider = Provider.of<LocationProvider>(
      context,
      listen: false,
    );
  ...
  Future<void> _syncFriendAnnotations() async {
    ...
    final Map<String, FriendLocationEntity> friends =
        _locationProvider?.friends ?? {};
    ...
    for (final entry in friends.entries) {
      ...
      final FriendLocationEntity friendData = entry.value;
  ...
  void _onMapTap(MapContentGestureContext gestureContext) async {
    ...
    for (final entry in _friendMapPoints.entries) {
      try {
        ...
        if (dx < 44 && dy < 44) {
          if (!mounted) return;
          final FriendLocationEntity? data =
              _locationProvider?.friends[entry.key];
